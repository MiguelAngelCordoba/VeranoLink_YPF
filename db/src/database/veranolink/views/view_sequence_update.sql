...
-- ===================================================================
    select
        ...
        case
            when nvl(wb.wbs_name, '~') != nvl(os.object_name, '~') then
                wb.wbs_name
...
-- INNER JOIN: solo se actualiza lo que ya existe en Ecosys
    ...
    where
        wb.wbs_code != os.object_code
        or nvl(wb.wbs_name, '~') != nvl(os.object_name, '~')
    union all
-- ===================================================================
...
--   B (linea base): valores de la LB vigente distintos a los ya enviados
--                   -> fechas OB, codigo y nombre
...
-- ===================================================================
    select
        ...
        case
            when nvl(ab.activity_name, '~') != nvl(os.object_name, '~') then
                ab.activity_name
        end                     as "CostObjectName",
        ...
        case
            when nvl(abs_.bl_start_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_start_date, timestamp '1900-01-01 00:00:00') then
                to_char(
        ...
        case
            when nvl(abs_.bl_finish_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_finish_date, timestamp '1900-01-01 00:00:00')
            then
                to_char(
        ...
        case
            when av.updatedate > os.update_sincronizado
                 and ( ab.activity_code != os.object_code
                       or nvl(ab.activity_name, '~') != nvl(os.object_name, '~')
                       or nvl(abs_.bl_start_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_start_date, timestamp '1900-01-01 00:00:00'
                       )
                       or nvl(abs_.bl_finish_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_finish_date, timestamp '1900-01-01 00:00:00'
                       ) ) then
                'CRONOGRAMA+LB'
            when av.updatedate > os.update_sincronizado then
                'CRONOGRAMA'
        ...
        json_object(
                'HierarchyPathID' value pe.path_ecosys
                    ...
                    case
                        when nvl(abs_.bl_start_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_start_date, timestamp '1900-01-01 00:00:00'
                        ) then
                            to_char(
                                nvl(ab.bl_start_date, ab.bl_finish_date),
                                'YYYY-MM-DD'
...
-- Datos de la LB que se sincronizo la ultima vez: permite comparar
-- LB nueva contra LB anterior y no reenviar las actividades sin cambios.
        left join act_bl           abs_ on abs_.project_baseline_id = os.project_baseline_id
                                 and abs_.activity_id = av.activity_id
    where
    -- Disparador A: avance en el cronograma actual
        av.updatedate > os.update_sincronizado
    -- Disparador B: algun valor de la linea base difiere de lo ya enviado
        or ab.activity_code != os.object_code
        or nvl(ab.activity_name, '~') != nvl(os.object_name, '~')
        or nvl(abs_.bl_start_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_start_date, timestamp '1900-01-01 00:00:00')
        or nvl(abs_.bl_finish_date, timestamp '1900-01-01 00:00:00') != nvl(ab.bl_finish_date, timestamp '1900-01-01 00:00:00');


-- sqlcl_snapshot {"hash":"ce7f461b31ce81287624bba5bc6a6128d889e03e","type":"VIEW","name":"VIEW_SEQUENCE_UPDATE","schemaName":"VERANOLINK","sxml":""}